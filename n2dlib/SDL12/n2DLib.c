...
inline unsigned short getPixelUnsafe(Texture_nKaruga *src, unsigned int x, unsigned int y)
{
	return src[x + y * src[0] + 3];
}

inline unsigned short getPixel(Texture_nKaruga *src, unsigned int x, unsigned int y)
{
	if(x < src[0] && y < src[1])
		return src[x + y * src[0] + 3];
	else
...
void drawSprite(Texture_nKaruga *src, int _x, int _y, int flash, unsigned short flashColor)
{
...
void drawSpritePart(Texture_nKaruga *src, int _x, int _y, const Rect* part, int flash, unsigned short flashColor)
{
	unsigned short c;
...
void drawSpriteScaled(Texture_nKaruga* source, const Rect* info, int flash, unsigned short flashColor)
{
...
void drawSpriteRotated(Texture_nKaruga* source, const Rect* sr, const Rect* rc, Fixed angle, int flash, unsigned short flashColor)
{
	...
	for(cp.y = fr.y; cp.y <= fr.h; cp.y++)
	{
		...
		for(cp.x = fr.x; cp.x <= fr.w; cp.x++)
		{
			...
			{
				setPixel(cp.x, cp.y, flash ? flashColor : currentPixel);
			}
			cdrp.x += dX;
			cdrp.y += dY;
		}
