...
void Joint::activate(Entity *_target, int _targetX, int _targetY, int _jointX, int _jointY, int _jointCX, int _jointCY, Texture_nKaruga *_timg, Texture_nKaruga *_jimg)
{
	int w, h;
	target = _target;
	timg = _timg;
	...
	targetX = _targetX;
	targetY = _targetY;
	Get_Size_Image(timg, NULL, NULL, &w, &h);
	targetCX = w / 2;
	targetCY = h / 2;
	jointX = _jointX;
	jointY = _jointY;
	Get_Size_Image(jimg, NULL, NULL, &w, &h);
	jointCX = _jointCX == -1 ? w / 2 : _jointCX;
	jointCY = _jointCY == -1 ? h / 2 : _jointCY;
}

void Joint::activate(Entity *_target, int _targetX, int _targetY, int _targetCX, int _targetCY, int _jointX, int _jointY, int _jointCX, int _jointCY, Texture_nKaruga *_timg, Texture_nKaruga *_jimg)
{
	int w, h;
	Get_Size_Image(jimg, NULL, NULL, &w, &h);
	target = _target;
